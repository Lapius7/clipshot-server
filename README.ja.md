...
このプロジェクトはまだビルド済みバイナリやコンテナイメージを公開していません（[ロードマップ](#ロードマップ既知の制約)参照）。[タグ付きリリース](https://github.com/Lapius7/clipshot-server/releases)はすべて**ソースのみ**です — `docker build`/`docker compose up` または `go build` で自分でビルドしてください。チェックサムを検証する対象となる別個の「公式バイナリ」は存在しないため、「改ざんされたバイナリをダウンロードしてしまったかどうか」という懸念のクラス自体を回避できますが、一方で実行するものの完全性は、クローンしたソースの完全性に依存することになります。

### クローンしたソースの検証

- **任意のコミットよりタグ付きリリースを優先してください。** `git clone` の後に `git checkout v0.1.0`（または該当のタグ）を行うことで、クローンした時点で `main` が何を含んでいるかに依存せず、固定された引用可能な履歴上の地点を得られます。
- **タグがGitHubの表示と一致しているか確認してください。** このリポジトリのタグは現時点でGPG署名されていないため（[ロードマップ](#ロードマップ既知の制約)参照）、今のところの「検証」とは暗号署名の検証ではなく、[Releasesページ](https://github.com/Lapius7/clipshot-server/releases)とGitHub自体の[コミット履歴](https://github.com/Lapius7/clipshot-server/commits/main)に表示されている内容と比較することを意味します。
- **`docker build` を信用する前にDockerfileを読んでください。** 短く、特に驚くようなことはありません（マルチステージビルド、`CGO_ENABLED=0`、非rootユーザーで実行）— ビルドコンテキストにアクセスするものなので、一度目を通す価値があります。

### まだビルド済みコンテナイメージが無い理由

`ghcr.io` への公開には、タグごとにイメージをビルド・署名するCIパイプラインが必要ですが、まだ構築されていません（[ロードマップ](#ロードマップ既知の制約)参照）。それまでは、タグ付きチェックアウトから `docker build -t clipshot-server .` を実行することが再現可能な代替手段です — CIパイプラインが使うのと同じDockerfileから、レジストリを信用する代わりに自分のマシンでビルドすることになります。
